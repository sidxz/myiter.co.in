...
<font face="Verdana, Verdana" color="#339900" size="6"><i>Notice List</i></font>
...
$query ="SELECT *FROM `official` WHERE `type` LIKE 'nb' ORDER BY opid DESC LIMIT 15";
	$result=mysqli_query($dbc,$query)
	or die("Oopzz! Our Server is over Crowded! Please retry  after sometine: ERROR CODE 161 ");
	
	while($row = mysqli_fetch_array($result))
	{
	echo '<a FONT class="type3" href="index.php?opid='.$row["opid"].'" target="_top">'.$row["topic"].'</a><br><font size="2"> by <b>'.$row["postedby"].'</b> on '.date('d M Y', strtotime($row["time"])).'</font><br>';
	}
?>
