...
HTML;
		...
		if(!empty($customs)){
			echo '<div class="custom-title">'.__('Below are the Sizes you have created','flashxml_admin').'</div>';
			foreach($customs as $custom){
				echo '<div class="custom-item"><span>'.$custom.'</span><input type="hidden" class="custom-item-value" value="'.$custom.'"/><input type="button" class="button" value="'.__('Delete','flashxml_admin').'"/></div>';
			}
		}
...
$options = array(
	array(
		"name" => __("Image Sizes",'flashxml_admin'),
		"desc" => __("The options listed below determine the dimensions in pixels to use in the shortcode of image.",'flashxml_admin'),
		"type" => "title"
	),
	array(
		"name" => __("Custom Sizes",'flashxml_admin'),
		"type" => "start"
	),
	array(
			"name" => __("Custom Sizes",'flashxml_admin'),
			"desc" => __("Enter the name of custom you'd like to create.",'flashxml_admin'),
			"id" => "customs",
