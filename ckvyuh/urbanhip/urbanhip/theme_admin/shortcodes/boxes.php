...
function theme_shortcode_message($atts, $content = null, $code) {
	...
	$align = $align?' '.$align:'';
	return '<div class="' . $code . $align . '"><div class="message_box_content">' . do_shortcode($content) . '</div></div>';
}
...
function theme_shortcode_framed_box($atts, $content = null, $code) {
	...
	return '<div class="' . $code .$rounded. '"'.$style.'><div class="framed_box_content"'.$content_style.'>' . do_shortcode($content) . '</div></div>';
}
