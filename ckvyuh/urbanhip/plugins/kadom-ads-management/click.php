...
/************************************************************************************************************/
	...
	$stat = $wpdb->get_results( "SELECT s.* 
									  FROM ".$kd_ads_stats." s 
									  WHERE s.id_campaign=".$id." AND s.date='".$date."'");
	if($stat[0] <> NULL){
		$update = $wpdb->get_results( "UPDATE ".$kd_ads_stats." SET clicks='".($stat[0]->clicks + 1)."', date='".$date."' WHERE id=".$stat[0]->id); 
	}
	else{
		$insert = $wpdb->get_results( "INSERT INTO ".$kd_ads_stats." VALUES(NULL,'".$date."','1','1',".$id.")"); 
	}
